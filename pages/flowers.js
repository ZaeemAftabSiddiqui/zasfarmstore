...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in flowers) {
      if (
        !flowers[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        flowers[item.title].color.push(item.color);
      }
      if (
        !flowers[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
      ...
      if (item.availableQty > 0) {
        flowers[item.title].color = [item.color];
        flowers[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(flowers)) }, // will be passed to the page component as props
  };
}
