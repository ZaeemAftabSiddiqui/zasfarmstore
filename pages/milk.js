...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "milk" });
  let milk = {};
  for (let item of products) {
    if (item.title in milk) {
      if (
        !milk[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        milk[item.title].color.push(item.color);
      }
      if (!milk[item.title].size.includes(item.size) && item.availableQty > 0) {
        milk[item.title].size.push(item.size);
      }
    } else {
      milk[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        milk[item.title].color = [item.color];
        milk[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(milk)) }, // will be passed to the page component as props
  };
}
export default milk;
