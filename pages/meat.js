...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "meat" });
  let meat = {};
  for (let item of products) {
    if (item.title in meat) {
      if (
        !meat[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        meat[item.title].color.push(item.color);
      }
      if (!meat[item.title].size.includes(item.size) && item.availableQty > 0) {
        meat[item.title].size.push(item.size);
      }
    } else {
      meat[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        meat[item.title].color = [item.color];
        meat[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(meat)) }, // will be passed to the page component as props
  };
}
export default meat;
