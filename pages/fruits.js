...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in fruits) {
      if (
        !fruits[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        fruits[item.title].color.push(item.color);
      }
      if (
        !fruits[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        fruits[item.title].size.push(item.size);
      }
    } else {
      fruits[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        fruits[item.title].color = [item.color];
        fruits[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(fruits)) }, // will be passed to the page component as props
  };
}
