...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in other) {
      if (
        !other[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        other[item.title].color.push(item.color);
      }
      if (
        !other[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        other[item.title].size.push(item.size);
      }
    } else {
      other[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        other[item.title].color = [item.color];
        other[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(other)) }, // will be passed to the page component as props
  };
}
