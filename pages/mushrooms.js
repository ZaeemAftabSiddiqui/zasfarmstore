...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "mushrooms" });
  let mushrooms = {};
  for (let item of products) {
    if (item.title in mushrooms) {
      if (
        !mushrooms[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        mushrooms[item.title].color.push(item.color);
      }
      if (
        !mushrooms[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        mushrooms[item.title].size.push(item.size);
      }
    } else {
      mushrooms[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        mushrooms[item.title].color = [item.color];
        mushrooms[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(mushrooms)) }, // will be passed to the page component as props
  };
}
